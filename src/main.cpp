...
uint256 hashGenesisBlock("0x00000100c91b0946ea6d2b3d57315ed87a26f702f525c1df16884932280099e8");//bbc
...
const string strMessageMagic = "bigboobscoin Signed Message:\n";
...
{
    // bigboobscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // bigboobscoin: 4 hours
static const int64 nTargetSpacing = 3 * 60; // bigboobscoin: 3 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // bigboobscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 3 * 60; // bigboobscoin: 3 minutes
        static const unsigned int TimeDaySeconds = 4 * 60 * 60; // bigboobscoin: 4 hours
...
{
    // bigboobscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd19ba5a0f9739e9df356cb6b6eaa50cc8f04c2634209882dd5c4a3ace92f8");//bbc
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "29Sept2017";//bbc
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1506692390;//bbc
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3220232;//bbc

        if (fTestNet)
        {
            block.nTime    = 1506692390;//bbc
            block.nNonce   = 2701105;//bbc
        }

 if (false && block.GetHash() != hashGenesisBlock)//bbc
        {
        ...
        assert(block.hashMerkleRoot == uint256("0x7b396ea80eaaa3f4f6f5dc98738fad7a1874feabee991e9df48dbe89defed5c6"));//bbc
...
unsigned char pchMessageStart[4] = { 0xff, 0xc4, 0xba, 0xdf }; // bigboobscoin: increase each by adding 7 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BigBoobsCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BigBoobsCoinMiner : ProcessBlock, block not accepted");
    }
